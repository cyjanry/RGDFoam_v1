...
) const
{
   ...
   // Info << "come to basicThermo::h()" << endl;
  return this->h(scalarField(this->p_.internalField(), cells), T, cells);
}
...
//    return tmp<scalarField>(NULL);
  ...
  return this->h(pp, T, this->p_.mesh().boundaryMesh()[patchi].faceCells());
}
...
Foam::tmp<Foam::scalarField> Foam::basicThermo::hs   // ADD this section inorder to make the phi for h available
(
    const scalarField& T,
    const label patchi
) const
...
return this->e(scalarField(this->p_.internalField(), cells), T, cells); //Jianhui Qi 19-12-2017
...
//    );
//    return tmp<scalarField>(NULL);
  const scalarField& pp = this->p_.boundaryField()[patchi];             //Jianhui Qi 19-12-2017
  return this->e(pp, T, this->p_.mesh().boundaryMesh()[patchi].faceCells());
}
...
) const
{
    ...
    const scalarField& pp = this->p_.boundaryField()[patchi];
    return this->Cp(pp, T, this->p_.mesh().boundaryMesh()[patchi].faceCells());
...
) const
{
   ...
   // );
   // return tmp<scalarField>(NULL);
return this->Cp(scalarField(this->p_.internalField(), cells), T, cells);
}


Foam::tmp<Foam::scalarField> Foam::basicThermo::Cp //Add by Jianhui Qi
(
	const scalarField& p,
	const scalarField& T,
	const labelList& cells
...
) const
{
   ...
   // );
   // return tmp<scalarField>(NULL);
       return this->Cv(this->p_.boundaryField()[patchi], T, patchi);
}
